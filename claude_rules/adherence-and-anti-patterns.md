...
Open issues confirm the failure this prevents: **#5502** documents Claude completing the work correctly but ignoring the CLAUDE.md workflow until explicitly asked "did you follow the CLAUDE.md instructions?" (closed not-planned — the fix is architectural, not a promised behavior change) (source: https://github.com/anthropics/claude-code/issues/5502). Per the official docs, CLAUDE.md sits at the *user-message* tier — the Output Styles comparison table states CLAUDE.md "[a]dds a user message after the system prompt" while `--append-system-prompt` "[a]ppends to the system prompt" — so for the handful of rules that truly need system-prompt priority, use `--append-system-prompt` (or managed `claudeMd`) instead (source: https://code.claude.com/docs/en/output-styles). **#6973** is a docs-clarification request (closed not-planned, no maintainer confirmation) articulating that same tier distinction (source: https://github.com/anthropics/claude-code/issues/6973). Canonical migration targets: a `protect-files.sh` `PreToolUse` hook that exits 2 on writes to `.env` / `package-lock.json` / `.git/`, a `block-rm-rf.sh` that denies dangerous Bash, and a `Stop` hook that runs the test suite and blocks the turn from ending until it passes (source: https://code.claude.com/docs/en/hooks-guide). Note the `Stop`-hook caveats: it must check `stop_hook_active` to avoid infinite loops, and Claude Code overrides it after **8 consecutive blocks** (raise via `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP`) (source: https://code.claude.com/docs/en/hooks-guide).
...
**The warning is the point:** any append-only loop produces **write-only knowledge** that bloats the store and degrades the very adherence it was meant to improve — the failure mode is documented on the tracker, where Claude "writes to" a **258-file** knowledge vault but "never reads from it" (source: https://github.com/anthropics/claude-code/issues/32161, closed as duplicate), and the self-improving-loop walkthrough itself warns that *"a self-evolving system that never prunes will eventually become noisy"* (source: https://www.mindstudio.ai/blog/self-evolving-claude-code-memory-obsidian-hooks). A self-improving loop is only safe paired with **scheduled pruning/archival** and a calibrated extraction prompt that excludes obvious/one-time items. Capture without pruning is not a feature; it is the bloat anti-pattern on a timer.
...
| 8 | Append-only self-improving / `Stop`-hook memory loops | Write-only knowledge bloats and is never retrieved (a 258-file vault written-to but never read, #32161) | Pair with scheduled pruning + calibrated extraction prompt | https://github.com/anthropics/claude-code/issues/32161; https://www.mindstudio.ai/blog/self-evolving-claude-code-memory-obsidian-hooks |
...
12. https://www.mindstudio.ai/blog/self-evolving-claude-code-memory-obsidian-hooks — (community) `Stop`-hook self-improving loop; "never prunes → noisy" write-only-knowledge warning + scheduled review/archival.
...
15. https://github.com/anthropics/claude-code/issues/6973 — (official issue, closed not-planned; docs-clarification request) corroborates the user-message vs system-prompt tier distinction (itself documented on the Output Styles page, #17); `--append-system-prompt` as the lever.
16. https://github.com/anthropics/claude-code/issues/32161 — (official issue, closed duplicate) a 258-file knowledge vault Claude "writes to … never reads from" — the write-only-knowledge failure mode.
17. https://code.claude.com/docs/en/output-styles — (official) the CLAUDE.md ("adds a user message after the system prompt") vs `--append-system-prompt` ("appends to the system prompt") tier distinction.
