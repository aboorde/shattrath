...
| #38487 | Even when working, scoping triggers via the pre-edit **Read** — so it loads on **edit** of an existing match but **not** on **Write** of a *new* file at a matching path; "blind to file creation — the exact moment when conventions matter most" | — | **closed not-planned** (March 2026) | (open GitHub issue, source: https://github.com/anthropics/claude-code/issues/38487) |
...
| Activation modes | Four: **Always**, **Auto Attached**, **Agent Requested**, **Manual** (driven by an `alwaysApply` toggle + `globs:`) | Two: unconditional (no `paths:`) vs path-scoped (`paths:`) |
...
6. https://github.com/anthropics/claude-code/issues/38487 — *(community / open GitHub issue, closed not-planned)* — blind to file creation (Read/Edit not Write).
