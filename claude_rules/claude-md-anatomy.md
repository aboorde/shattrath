...
- **`CLAUDE_CODE_NEW_INIT=1`** enables an interactive multi-phase flow — now **officially documented**: `/init` "asks which artifacts to set up: CLAUDE.md files, skills, and hooks," then "explores your codebase with a subagent, fills in gaps via follow-up questions, and presents a reviewable proposal before writing any files" (source: https://code.claude.com/docs/en/memory). It configures from intent (which **recurring tasks become skills**, which **unsafe operations get hooks**) rather than file inspection alone; a community write-up adds that the interview uses the `AskUserQuestion` tool (community) (source: https://medium.com/@AdithyaGiridharan/claude-codes-init-command-is-getting-a-brain-transplant-e0c389ec0c6b).
...
## Sources
1. https://code.claude.com/docs/en/memory — *(official)* memory file locations, load order, upward walk, concatenation order, on-demand subdir loading, HTML-comment stripping, `CLAUDE.local.md` semantics (a supported local tier that "loads alongside `CLAUDE.md` and is treated the same way"; gitignore it; home-dir `@`-import for cross-worktree sharing), `@path` imports (inline-at-launch, 4-hop, path resolution, approval dialog), context-not-config.
...
- **Version-sensitive / may be stale:** `CLAUDE_CODE_NEW_INIT=1`'s env var and multi-phase flow are now confirmed in the official memory docs (artifacts = CLAUDE.md/skills/hooks; subagent exploration; reviewable proposal before writing); only the community "March 2026" date framing is unsourced to an official changelog. The `@path` semantics, 4-hop limit, and `CLAUDE.local.md` semantics (a supported local tier) are current as of 2026-06-04 and could change; re-verify against https://code.claude.com/docs/en/memory and the changelog. Whether the positive-framing gains were measured *specifically* on Opus 4.8 (vs 4.6/4.7) is not established; the literalism note implies scope-explicitness matters more than ever, but no A/B was found.
