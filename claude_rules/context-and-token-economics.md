...
A bigger window does **not** escape context rot. The 1M-token window (Opus 4.6+/Sonnet 4.6 select plans; compaction works the same at the larger limit) (source: https://code.claude.com/docs/en/context-window) shifts the gradient but does not remove it: Chroma observes a clear effect around **300K–400K** on 1M-token models, and noticeable degradation possible by **~50K** on a 200K-window model (source: https://www.trychroma.com/research/context-rot). Filling a 1M window with rules and reads "because it fits" trades budget for behavior.
...
7. https://www.trychroma.com/research/context-rot (community) — 18-model degradation study, 300K–400K and ~50K thresholds.
...
- **Community / preprint, labeled in-line:** Chroma's 18-model study and its 300K–400K / ~50K thresholds (2025-07, Claude 4 era — thresholds for Opus 4.7/4.8 are unverified and likely shifted); the ETH Zurich AGENTS.md >20%-cost / reduced-success result (preprint arXiv 2602.11988 — detailed sub-numbers are preprint/secondary). Do not attribute these to the separate descriptive study arXiv 2509.14744.
